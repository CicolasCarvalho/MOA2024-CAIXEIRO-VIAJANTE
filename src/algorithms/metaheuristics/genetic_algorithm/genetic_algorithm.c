...
static void elite_selection(Population *population, Gene **chromosome);
...
Path *build_genetic_algorithm(Graph *graph, size_t from, CrossoverAlgorithm crossoverAlgorithm, int population_size, double max_time) {
    ...
    while (stable_gens < GEN_MAX_STABLE && get_time() < start_time + max_time) {
        actual_gen++;
        ...
        Population_mutate(population, (GEN_MUT_RATE / (double)population->size));
...
static void apply_PMX_crossover(Population *population) {
    ...
    for (size_t i = GEN_ELITE_COUNT; i < population->size; i += 2) {
        ...
        Gene *a = Population_selection(population, RANKING_SELECTION, &last_selection),
             *b = Population_selection(population, RANKING_SELECTION, &last_selection);
        ...
        size_t i1 = get_rand(population->length),
               i2 = i1 + get_rand(population->length - i1);
        ...
        for (
            ...
            while (k < population->length && c[k] != -1) k++;
            while (l < population->length && d[l] != -1) l++;

            if (k < population->length && !InclusionMap_has(segment_map1, b[j])) {
                c[k] = b[j];
                InclusionMap_set(&segment_map1, b[j]);
            }

            if (l < population->length && !InclusionMap_has(segment_map2, a[j])) {
                d[l] = a[j];
    ...
    // for (size_t i = 0; i < population->size; ++i) {
    //     // Chromosome_print(population->chromosomes[i], population->length);
    // }
}
...
static void apply_OX1_crossover(Population *population) {
    ...
    for (size_t i = GEN_ELITE_COUNT; i < population->size; i += 2) {
        ...
        for (
            size_t j = 0, k = (i2 + 1) % population->length, l = (i2 + 1) % population->length;
            j < population->length;
            ++j
        ) {
            while ((k < i1 || k > i2) && c[k] != -1) k = (k + 1) % population->length;
            while ((l < i1 || l > i2) && d[l] != -1) l = (l + 1) % population->length;
...
static void Population_mutate(Population *population, double rate) {
    for (size_t i = 0; i < population->size; ++i) {
        for (size_t j = 0; j < population->length; ++j) {
            int64_t mut = (int64_t)get_rand((size_t)(1 / rate));

            if (mut == 1) {
                size_t a = get_rand(population->length);
                size_t b = get_rand(population->length);
